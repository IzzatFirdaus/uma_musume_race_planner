...
/**
 * Uma Musume Race Planner API — Plan Predictions
 * GET action: returns race_predictions rows for a given plan id.
 * Response: { success: true, predictions: array, request_id: string }
 */
...
switch ($action) {
    case 'get':
        ...
        try {
            ...
            send_json(200, ['success' => true, 'predictions' => $predictions]);
        } catch (Throwable $e) {
            if (isset($log)) {
                $log->error('Failed to fetch plan predictions (api)', [
                    'plan_id' => $plan_id,
