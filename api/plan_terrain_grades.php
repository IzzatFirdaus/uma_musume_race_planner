...
/**
 * Uma Musume Race Planner API — Plan Terrain Grades
 * GET action: returns terrain_grades for a given plan id.
 * Response: { success: true, terrain_grades: array, request_id: string }
 */
...
switch ($action) {
    case 'get':
        ...
        try {
            ...
            send_json(200, ['success' => true, 'terrain_grades' => $terrain_grades]);
        } catch (Throwable $e) {
            if (isset($log)) {
                $log->error('Failed to fetch plan terrain grades (api)', [
                    'plan_id' => $plan_id,
