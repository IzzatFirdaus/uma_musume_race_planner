...
test.describe("Accessibility - WCAG 2.2 Compliance", () => {
    ...
    test("homepage should have no critical axe violations", async ({
        page,
    }) => {
        ...
        if ((axeResults.violations || []).length > 0) {
            console.log(
                `Total violations found: ${axeResults.violations.length}`,
            );
    ...
    test("color contrast meets WCAG AA standards", async ({ page }) => {
        ...
        try {
            ...
            console.warn(
                "Axe-core check skipped. Checking text contrast manually.",
            );
            axeResults = { violations: [] };
        }
        ...
        if (contrastViolations.length > 0) {
            console.log(
                `⚠ Color contrast issues found: ${contrastViolations.length}`,
            );
    ...
    test("images have alt text", async ({ page }) => {
        ...
        for (const img of images) {
            ...
            const hasAccessibility =
                alt || ariaLabel || role === "presentation";
            expect(hasAccessibility).toBeTruthy();
        }

        console.log(
            `✓ All ${images.length} images have accessibility attributes`,
        );
    });
